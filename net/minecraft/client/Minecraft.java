...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    private static final ResourceLocation locationMojangPng = new ResourceLocation("Images/loading.jpg");
    ...
    private void drawSplashScreen(TextureManager textureManagerInstance) throws LWJGLException {
        ...
        // Calculate image dimensions to fill the screen while maintaining 16:9 aspect ratio
        int screenWidth = scaledResolution.getScaledWidth();
        int screenHeight = scaledResolution.getScaledHeight();
        double aspectRatio = 16.0 / 9.0;
        int imageWidth, imageHeight;

        if (screenWidth / (double) screenHeight > aspectRatio) {
        ...
        worldRenderer.pos(x, y + imageHeight, 0.0D).tex(0.0D, 1.0D).color(255, 255, 255, 255).endVertex();
        worldRenderer.pos(x + imageWidth, y + imageHeight, 0.0D).tex(1.0D, 1.0D).color(255, 255, 255, 255).endVertex();
        worldRenderer.pos(x + imageWidth, y, 0.0D).tex(1.0D, 0.0D).color(255, 255, 255, 255).endVertex();
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Mouse.next())
            {
                ...
                if (i1 <= 200L)
                {
                    ...
                    if (j != 0)
                    {
                        if (GameSettings.isKeyDown(this.gameSettings.ofKeyBindZoom))
                        {
                            // Suppress hotbar scroll while zooming
                            j = 0;
                        }
            ...
            while (Keyboard.next())
            {
                ...
                {
                    KeyBinding.onTick(k);
                    if(k == Keyboard.KEY_U && !(currentScreen instanceof EchoGui))
                        displayGuiScreen(new EchoGui());
                }
