...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private float getFOVModifier(float partialTicks, boolean useFovSetting)
    {
        ...
        if (useFovSetting)
        {
            baseFov = this.mc.gameSettings.fovSetting;

            if (Config.isDynamicFov())
            {
                baseFov *= this.fovModifierHandPrev + (this.fovModifierHand - this.fovModifierHandPrev) * partialTicks;
            }
        }

        boolean zoomKeyHeld = false;

        if (this.mc.currentScreen == null)
        {
            zoomKeyHeld = GameSettings.isKeyDown(this.mc.gameSettings.ofKeyBindZoom);
        }

        // Handle mouse scroll input while zooming
        if (Mouse.hasWheel())
        {
            int scroll = Mouse.getDWheel();
            if (scroll != 0 && zoomKeyHeld)
            {
                float scrollStep = 0.062F;
                if (scroll > 0)
                {
        ...
        float fov = baseFov * zoomFovMultiplier;
        ...
        if (entity instanceof EntityLivingBase)
        {
            ...
            {
                float deathProgress = living.deathTime + partialTicks;
                fov /= (1.0F - 500.0F / (deathProgress + 500.0F)) * 2.0F + 1.0F;
            }
        }

        // Water FOV adjustment
